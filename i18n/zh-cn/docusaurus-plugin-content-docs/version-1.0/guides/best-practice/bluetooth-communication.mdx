...
「设备应用」通过 `hmBle` API 来使用蓝牙能力，「伴生服务」通过 `Messaging API` 来使用蓝牙能力，目前两者都只能够操作二进制数据，对开发者来说数据结构的组织较为繁琐。

参考示例 [ToDoList](../../samples/app/toDoList.md)，其中 `/shared/message.js` 中对 `hmBle` 和 `Messaging API` 进行封装，更加便于开发者开发应用。
