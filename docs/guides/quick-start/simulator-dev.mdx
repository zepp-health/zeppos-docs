...
Let's navigate to the `hello-world` directory using the terminal. Once there, run the [`zeus dev` compile preview command](../../guides/tools/cli/index.md#zeus-dev-compilation-preview-simulator) compile preview command. This instructs the Zeus CLI to compile the project code and initiate a preview via the simulator.
