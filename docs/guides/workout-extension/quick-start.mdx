...
The process of "Workout Extension" is the same as that of Zepp OS Mini Program, and the required power builder is the same. Please refer to [Environment Preparation](../quick-start/environment.mdx) to complete the environment preparation.
...
We use the Terminal to enter the newly created "Workout Extension" directory, execute the `zeus dev` compile preview command, and we can see our workout extension application in the Simulator.
...
This process is consistent with the Zepp OS Mini Program. Refer to [Preview on a Watch](../quick-start/preview.mdx) to complete the real machine preview.
...
The `widgets` field is an array with a maximum length of `5`. That is to say, in the "Workout Extension" application with the same appId, up to `5` "Workout Extension" plugins can exist at the same time. Users can add multiple "Workout Extension" plugins in the "Workout" application, and the maximum number added at the same time depends on the model performance, at least `2`.
...
- The drawing area is limited, and UI widgets can only be drawn within the boundaries of the device screen. For example, the drawing area size of "Workout Extension" on Amazfit Balance devices is consistent with the screen resolution of 480 x 480 px.
