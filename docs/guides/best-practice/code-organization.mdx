...
The `class.js` (logic-related classes) in our code and the tool functions in the `/utils` directory can be run in the test framework, which saves a lot of time when writing pure JavaScript parts without relying on a simulator or a real machine, and allows us to debug the code in the test framework, thus improving development efficiency.
